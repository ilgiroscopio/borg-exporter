...
source ~/.borgexporterconfig

for index in "${!BORG_REPO_NAMES[@]}"; do
    BORG_REPO_NAME=${BORG_REPO_NAMES[$index]}
    BORG_REPO_DISPLAY_NAME=${BORG_REPO_DISPLAY_NAMES[$index]}
    BORG_PASSPHRASE=${BORG_PASSPHRASES[$index]}
    ...
    {
      echo "borg_backups_total{repository=\"$BORG_REPO_DISPLAY_NAME\"} $ARCHIVES_COUNT"
      echo "borg_last_backup_timestamp{repository=\"$BORG_REPO_DISPLAY_NAME\"} $LATEST_ARCHIVE_TIMESTAMP"
      echo "borg_deduped_size{repository=\"$BORG_REPO_DISPLAY_NAME\"} $DEDUPLICATED_SIZE"
